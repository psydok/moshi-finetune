...
@dataclass
class zero_model_state:  # noqa: N801
    buffers: dict
    param_shapes: dict
    shared_params: list
    ds_version: int
...
def _zero2_merge_trainable_params(state_dict, world_size, fp32_flat_groups, zero_model_states):
    ...
    for shapes, full_single_fp32_vector in zip(
        param_shapes, merged_single_partition_of_fp32_groups, strict=False
    ):
        offset = 0
        ...
        def zero2_align(x):
            return align_to * math.ceil(x / align_to)  # noqa: B023
...
def convert_zero_checkpoint_to_fp32_state_dict(
    ...
    if max_shard_size is not None:
        ...
        StateDictSplit = typing.NamedTuple("StateDictSplit", [("is_sharded", bool), ("filename_to_tensors", str)])
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--moshi_lm_kwargs_path", type=str, help="Path to the Moshi LM kwargs (json file)"
    )
    ...
    # copy the Moshi LM kwargs to the output directory
    if args.moshi_lm_kwargs_path:
        moshi_lm_kwargs_output_path = os.path.join(args.output_dir, "moshi_lm_kwargs.json")
        os.system(f"cp {args.moshi_lm_kwargs_path} {moshi_lm_kwargs_output_path}")
